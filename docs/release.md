...
`versionName` `0.1.2`, `versionCode` `3`. Bump **both** on every store-less
sideload.
...
Replace `0.1.2` / `v0.1.2` with the version you are shipping. First public tag
was `v0.1.0`.
...
3. On GitHub → [Releases](https://github.com/rururunu/AnyaAndroid/releases),
   ...
   - Attach `Anya-v0.1.2.apk` and `latest.json` (filename must stay `latest.json`)
...
| Field         | Required | Role                                      |
| ------------- | -------- | ----------------------------------------- |
| `version`     | yes      | Compared to `versionName` (`0.1.2`)       |
