...
| `:feature:approval`      | android + compose | 工具 / ask_user / 计划卡                        |
...
flowchart TB
  subgraph Phone["Anya Companion"]
    Prefs[DataStore: 主机 · 令牌 · deviceId]
...
深度链接：`anya://pair?…`（主机、端口、令牌、可选公网主机）。首次 `hello` 成功后，手机把设备凭证写入 DataStore，之后启动复用，直到用户解除配对。
